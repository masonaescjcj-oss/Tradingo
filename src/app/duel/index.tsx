...
export default function DuelHubScreen() {
  ...
  return (
    <Screen>
      <BackHeader title={t('دوئل چارتون')} caption={t('سه راند، سه مهارت')} />
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.hero}>
          <View style={styles.heroHead}>
            ...
            <View style={{ flex: 1, gap: 2 }}>
              ...
              <Txt w={700} size={13} color={colors.text2}>
                {stats?.played
                  ? t('{wins} برد · {losses} باخت · {ties} مساوی', { wins: fa(stats.wins), losses: fa(stats.losses), ties: fa(stats.ties) })
                  : t('اولین دوئلت رو بزن!')}
              </Txt>
            </View>
          </View>
          {ROUNDS.map((r, i) => (
          ...
          <Txt w={700} size={11.5} color={colors.text3}>
            {rewardsLeft
              ? t('امروز {n} دوئل دیگه سکه و امتیاز جایزه داره.', { n: fa(rewardsLeft), count: rewardsLeft })
              : t('جایزه‌ی دوئل‌های امروز تموم شده؛ ولی بازی‌کردن آزاده.')}
          </Txt>
        </View>

        <Section title={t('با شمعک')}>
          <View style={styles.card}>
            <View style={styles.row}>
              <CoachAvatar size={44} />
              <Txt w={700} size={13} lh={1.7} color={colors.text2} style={{ flex: 1 }}>
                {t('هر وقت کسی نبود، شمعک پایه‌ی دوئله. سطحش رو انتخاب کن.')}
              </Txt>
            </View>
              ...
              {BOT_LEVELS.map((l) => {
                ...
                return (
                  <Pressable
                    ...
                    accessibilityLabel={`${t(l.label)}: ${t(l.hint)}`}
            ...
            <Button3D label={t('دوئل با شمعک')} onPress={() => router.push(`/duel/play?mode=bot&level=${level}`)} />
          </View>
        </Section>

        <Section title={t('با دوستت')}>
          <View style={styles.card}>
            <Txt w={700} size={13} lh={1.7} color={colors.text2}>
              {t('اول خودت بازی کن، بعد لینک دوئل رو برای دوستت بفرست. اون همون راندها رو بازی می‌کنه و نتیجه برای هر دوتون میاد.')}
            </Txt>
            {online === false ? (
              <Txt w={700} size={12.5} color={colors.gold}>
                {t('دوئل با دوست هنوز روی سرور فعال نشده.')}
              </Txt>
            ) : null}
            <Button3D variant="gold" label={t('ساختن دوئل و دعوت دوست')} onPress={() => router.push('/duel/play?mode=friend')} disabled={online === false} />
            <View style={styles.join}>
              <TextInput
                value={code}
                onChangeText={(text) => setCode(normalizeCode(text))}
                placeholder={t('کد دوئل دوستت')}
                placeholderTextColor={colors.faint}
                autoCapitalize="characters"
                autoCorrect={false}
                maxLength={8}
                accessibilityLabel={t('کد دوئل')}
                style={[styles.input, !code && styles.inputEmpty]}
                onSubmitEditing={() => joinCode.length === 6 && router.push(`/duel/${joinCode}`)}
              />
              <Button3D label={t('برو')} height={46} radius={12} edge={4} size={15} disabled={joinCode.length !== 6} onPress={() => router.push(`/duel/${joinCode}`)} />
            </View>
          </View>
        </Section>

        {signedIn && online ? (
          <Section title={t('دوئل‌های من')}>
            {list == null ? (
              <Txt w={700} size={13} color={colors.text3}>
                {t('در حال گرفتن دوئل‌ها…')}
              </Txt>
            ) : list.length === 0 ? (
              <Txt w={700} size={13} color={colors.text3}>
                {t('هنوز دوئلی با دوستات نداشتی.')}
              </Txt>
            ) : (
...
function DuelRow({ duel }: { duel: DuelSummary }) {
  ...
  const label =
    duel.status === 'expired'
      ? t('منقضی شد')
      : result
        ? t('{outcome}، {mine} به {theirs}', {
            outcome: result.outcome === 'win' ? t('بردی') : result.outcome === 'loss' ? t('باختی') : t('مساوی'),
            mine: fa(result.mine),
            theirs: fa(result.theirs),
          })
        : duel.role === 'creator'
          ? t('منتظر حریف')
          : t('در جریان');
  return (
    <Pressable
      ...
      accessibilityLabel={t('دوئل {code}: {label}', { code: duel.code, label })}
    ...
    >
      ...
      <View style={{ flex: 1, gap: 2 }}>
        <Txt w={900} size={14.5} numberOfLines={1}>
          {duel.otherName ?? t('هنوز کسی بازی نکرده')}
        </Txt>
