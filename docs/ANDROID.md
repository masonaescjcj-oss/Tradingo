...
اندازه‌ی شمعک توی اسپلش از `imageWidth` افزونه‌ی `expo-splash-screen` توی `app.json` (۱۸۰) میاد و `SPLASH_MASCOT` توی `Intro.tsx` از روی همون حساب شده؛ اگه یکی عوض شد، اون یکی هم باید عوض بشه.
